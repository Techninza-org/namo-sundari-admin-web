    ...
    async (page) => {
      ...
      try {
        const { data } = await axios.get(
          ...
          }/admin/get-user-orders?userId=${id}&page=${page}&limit=10`,
          {
            headers: {
  ...
  const columns = [
    {
      key: "order_id",
      label: "Order ID",
      render: (row) => <div>{row.order_id || "N/A"}</div>,
      width: "w-60",
    },
    {
      key: "user_name",
      label: "Name",
      render: (row) => <div>{row.user_name || "N/A"}</div>,
      width: "w-48",
    },
    {
      key: "user_mobile",
      label: "Mobile",
      render: (row) => <div>{row.user_mobile || "N/A"}</div>,
      width: "w-40",
    },
    {
      key: "total_amount",
      label: "Total Amount",
      render: (row) => <div>{row.total_amount || "N/A"}</div>,
      width: "w-32",
    },
